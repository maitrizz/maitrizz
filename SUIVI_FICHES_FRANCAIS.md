...
> **CHANTIER 2 TERMINÉ pour les 18 notions de grammaire/lexique (02 → 19), le 28/06/2026.** Chaque banque = 6 Fondamentaux / 10 Concours / 5 Expert (≈ 21 exercices), 0 `TODO refonte` restant, `tsc`/`eslint` verts, 0 tiret cadratin dans les données, rendu HTTP 200 en L3 et M2. **À FAIRE** : validation humaine à l'écran (notions 03 → 19) ; la 02 est déjà validée. Restent ensuite, hors grammaire/lexique : la **méthodologie (00)** et la **Partie 3 (20 Produire, 21 Référentiel)**, à arbitrer (recueil/méthode : format différent des banques d'exercices).
