...
export async function generateStaticParams() {
  return MATIERES.flatMap((matiere) =>
    getFicheMetas(matiere).map((fiche) => ({ matiere, slug: fiche.slug }))
  );
}
...
export async function generateMetadata({
  ...
  if (!isValidMatiere(matiere)) return {};

  const fiche = getFiche(matiere, slug);
  if (!fiche) return {};

  const url = `/reviser/epreuves/ecrites/${matiere}/${slug}`;
  return {
...
export default async function FichePage({
  ...
  const { prev, next } = getAdjacentFiches(matiere, slug);
  const base = `/reviser/epreuves/ecrites/${matiere}`;
    ...
    <div className="min-h-screen bg-base-100">
      ...
      <div className="max-w-4xl mx-auto px-6 lg:px-8 py-8 flex flex-col gap-6">
        ...
        <div className="flex items-center gap-2 text-base-content/50 text-sm flex-wrap">
          ...
          <span>›</span>
          <Link href="/reviser/epreuves" className="hover:text-primary transition-colors">Épreuves</Link>
          <span>›</span>
          <Link href="/reviser/epreuves/ecrites" className="hover:text-primary transition-colors">Écrites</Link>
          <span>›</span>
