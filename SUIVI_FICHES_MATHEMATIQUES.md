...
4ter. **Dérivation L3 — phase 1 (fait, économe)** : `MATHEMATIQUES_L3` câblée = méthodologie L3 + les 27 notions **partagées** avec le M2 (liste `MATHEMATIQUES_NOTIONS`). La méthodologie est désormais une **fabrique** `ficheMethodologie(niveau)` : variante L3 = épreuve commune français+maths 4 h, coeff 5, maths 10 pts, note ≤ 2,5/10 par partie éliminatoire, programme cycle 4, Arrêté du 17 avril 2025 ; le contenu « Seconde » est signalé comme hors-programme pour le L3. **Phase 2 — en cours.** **Fiche Vecteurs créée** (`vecteurs.ts`, numéro 28, L3 uniquement, insérée en fin de Partie 4 après Transformations ; périmètre cycle 4 2025 : définition/égalité, lien translation, somme + relation de Chasles, produit par un réel + colinéarité, coordonnées/norme/milieu ; déterminant et produit scalaire marqués « Seconde » ; sources : programme cycle 4 2025 education.gouv.fr). L3 = 29 fiches, M2 = 28 (inchangé). **Reste à faire** : retirer le contenu « Seconde » (suites entière, intervalles/valeur absolue, quartiles/variance) et le bloc sphère (fiche 26) des variantes L3 — ces fiches sont aujourd'hui encore identiques au M2 (passages « Seconde » étiquetés hors-programme).
